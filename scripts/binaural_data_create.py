...
class SubGraph_sampling():
  def __init__(self, points_to_sample=100, plotting=False):
        ...
        self.filedir = '/checkpoint/sagnikmjr2002/code/ActiveAVSepMovingSource/data/audio_data/libriSpeech100Classes_MITMusic_ESC50/1s_chunks/train_preprocessed'
...
directory = "/checkpoint/sagnikmjr2002/code/ActiveAVSepMovingSource/data/active_datasets/v1/train_731243episodes/content"
...
DUMP_FP = f"{DUMP_DR_ROOT}/train_locationPredictor/{NUM_EPISODES_PER_SCENE}episodesPerScene.json"

list_all_wavs = []
if os.path.isfile(DUMP_FP):
    with open(DUMP_FP, "r") as fi:
        list_all_wavs = json.load(fi)["train"]
...
for ite, filename in enumerate(tqdm(os.listdir(directory))):
    ...
    if 'moving_source' not in file_path:
        with gzip.open(file_path, "rb") as f:
            ...
            for i in range(len(list_episodes)):  # :, 2
