...
class HabitatSimAudioEnabled(HabitatSim):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self._moving_source_indices = None
        self._receiver_position_index = None
        self._agent_start_index = None
    ...
    def reconfigure(self, config: Config) -> None:
        ...
        self._agent_start_index = self._position_to_index(self.config.AGENT_0.START_POSITION)
        
        #print("**********************************************************************self.config.AGENT_0.AUDIO_SOURCE_POSITIONS ", self.config.AGENT_0.AUDIO_SOURCE_POSITIONS[0])
        #print("*******************************************************************************************self.current_scene_name is ", self.current_scene_name)
        #print(abcd)
        ...
        if IS_MOVING_SOURCE:
            self._moving_source_indices = self.config.AGENT_0.MOVING_SOURCE_POSITIONS # added by us
            self.moving_source_pointer = 0 # added by us

            one_step_ahead_pointer = self.moving_source_pointer + 1
            if one_step_ahead_pointer >= len(self._moving_source_indices):
                one_step_ahead_pointer = len(self._moving_source_indices) - 1
            self._one_step_ahead_source = self._moving_source_indices[one_step_ahead_pointer]
    ...
    def reset(self):
        ...
        if IS_MOVING_SOURCE:
            self.moving_source_pointer = 0 # added by us
            self._source_position_indices[0] = self._moving_source_indices[0] # added by us
    ...
    def step(self, action, only_allowed=True):
        ...
        if IS_MOVING_SOURCE:
            self.moving_source_pointer += 1 # added by us
        
            if self.moving_source_pointer >= len(self._moving_source_indices): # added by us
                self.moving_source_pointer = len(self._moving_source_indices) - 1 # added by us
            self._source_position_indices[0] = self._moving_source_indices[self.moving_source_pointer] # added by us
        ...
        # print("#################### agent_loc ", self.graph.nodes[self._receiver_position_index]['point']) # USE THIS
        # print("#################### mov_src_loc ", self.graph.nodes[self._source_position_indices[0]]['point']) # USE THIS
    ...
    def get_current_ground_truth_deltax_deltay(self):
        ...
        try:
            ...
            # print('source didnt move')
            mov_src_loc = self.graph.nodes[self._position_to_index(self.config.AGENT_0.AUDIO_SOURCE_POSITIONS[0])]['point']
        ...
        if self._one_step_ahead_source is not None:
            try:
                ...
                future_mov_src_loc = mov_src_loc
                print("@@@@@@@@@@@@ future_mov_src_loc = mov_src_loc")
        else:
            raise ValueError()
