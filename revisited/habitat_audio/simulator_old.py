...
class HabitatSimAudioEnabled(HabitatSim):
    ...
    def get_current_ground_truth_deltax_deltay(self):
        ...
        agent_loc = self.graph.nodes[self._receiver_position_index]['point']
        mov_src_loc = self.graph.nodes[self._source_position_indices[0]]['point']

        return np.array([np.abs(agent_loc[0] - mov_src_loc[0]), np.abs(agent_loc[2] - mov_src_loc[2])]) # format is np.array([delta_x, delta_y])
