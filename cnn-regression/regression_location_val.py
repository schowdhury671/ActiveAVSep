...
class BinauralRIRdataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gt_bin_mono_fname = folder_name + '/' + bin_rir_fname.replace("/", "_")[:-4].split("binaural_rirs")[-1] + '_' + mono_fname.replace("/", "_")[:-4].split("audio_data")[-1] + '.pt'   #folder_name + '/' + bin_rir_fname.replace("/", "_")[:-4] + '_' + mono_fname.replace("/", "_")[:-4] + '.pt'
...
MAX_VAL = 2.90 # set the value = target max of training labels
...
device_ids = [0]   #[0,1,2,3,4,5,6,7] # for 8 gpus
...
try:
    model.load_state_dict(torch.load(root_dir + '/best_train_ckpt.pth')['state_dict'])
