...
CLASS_NAMES_TO_LABELS = {'7540': 41, '3274': 78, '5727': 82, '5157': 9, '3436': 67, '4731': 47, '4057': 1, '3851': 72,
                         '7932': 15, '597': 80, '119': 58, '345': 10, '460': 45, '868': 33, '909': 56, '125': 37,
                         '203': 50, '667': 32, '1578': 66, '2194': 44, '2137': 65, '2299': 8, '2319': 21}
'''
...
class PassiveDataset(Dataset):
    def __init__(self, split="train", scene_graphs=None, sim_cfg=None,):
        ...
        #self.audio_dir = '/fs/nexus-projects/ego_data/active_avsep/move2hear-active-AV-separation/data/audio_data/VoxCelebV1TenClasses_MITMusic_ESC50/train_preprocessed'    
        ...
        #import pdb; pdb.set_trace()
        # self.audio_files = self.audio_files
        ...
        for audio_file in self.audio_files:
            ...
            try:
              ...
              temp_list.append(audio_file.split("_")[0])
            except:
              pass  
        ...
        #   print(type(scene), scene)
          ...
          try:
              #self.sourceAgentLocn_datapoints_dir = "/fs/nexus-projects/ego_data/active_avsep/mp3d/"
              with open(os.path.join(self.sourceAgentLocn_datapoints_dir,  scene + ".pkl"), "rb") as fi:
                  ...
                  #  print("************* sourceAgentLocn_datapoints is ", sourceAgentLocn_datapoints) 
  
              if split.split("_")[0] == "train":
              
                #   print("************* train sourceAgentLocn_datapoints len is 1 ", len(sourceAgentLocn_datapoints))
                #   print("***********self.audio_cfg.NUM_PASSIVE_DATAPOINTS_PER_SCENE is ",self.audio_cfg.NUM_PASSIVE_DATAPOINTS_PER_SCENE)   
                  sourceAgentLocn_datapoints = sourceAgentLocn_datapoints[:self.audio_cfg.NUM_PASSIVE_DATAPOINTS_PER_SCENE]
                #   print("************* train sourceAgentLocn_datapoints len is 2 ", len(sourceAgentLocn_datapoints))
                  ...
                  #print("************* val sourceAgentLocn_datapoints len is ", len(sourceAgentLocn_datapoints))
                  
              
              for datapoint in sourceAgentLocn_datapoints:
                  audio_files, target_class = self.get_target_class_audio_files_for_sources()
                  
                  #print("************* audio_files type is ", type(audio_files))
                  #print("************* target class type is ", type(target_class))
                  
                  
                  self.target_classes.append(target_class)
                  
                  #print("************* target class len is ", len(self.target_classes))
                  ...
                  for idx, source_locn in enumerate(all_source_locns):
                      ...
                      try:
                        ...
                        complete_datapoint.append((binaural_rir_file, audio_file, delta_x_az, delta_y_az))
                        #print("***** complete_datapoint entry is ", complete_datapoint[-1])
                      except:
                        pass
        ...
        # print("************* self.complete_datapoint_files len is ", len(self.complete_datapoint_files))  # 248858
        ...
        # print("************source agent location datapoints len ", len(sourceAgentLocn_datapoints))  # 1984
    ...
    def load_source_audio(self):
        for audio_file in tqdm(self.audio_files):
            ...
            # sr = 2  #16000
            # audio_data = np.random.rand(2)   #torch.load('dummy_audio.pt')[0:2] 
    ...
    def compute_audiospects(self, rirs_audio, target_class):
        ...
        return np.log1p(mixed_mag), gt_bin_mag, gt_mono_mag, np.array([target_class]).astype("int64"), delta_x_az, delta_y_az
