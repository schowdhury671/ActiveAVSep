...
class PPOTrainer(BaseRLTrainer):
    ...
    def __init__(self, config=None, val_config=None):
    
        super().__init__(config, val_config)
        
        #print("Inside PPO trainer init")
        ...
        if self.val_config.RL.PPO.use_ddppo:
            interrupted_state = load_interrupted_state()
            if interrupted_state is not None:
                self.val_config = interrupted_state["config"]
    ...
    def _setup_actor_critic_agent(self, is_eval=False) -> None:
        ...
        assert cfg.EXTRA_DEPTH or cfg.EXTRA_RGB, "set at least one of EXTRA_RGB and EXTRA_DEPTH to true"
        ...
        self.actor_critic = AAViDSSPolicy(
            ...
            extra_rgb=cfg.EXTRA_RGB,
            extra_depth=cfg.EXTRA_DEPTH,
            ppo_cfg=ppo_cfg,
        )
    ...
    def _collect_rollout_step(
        ...
        # except:
        #     torch.save({'actions':actions,'outputs':[]},'check_waste/dummy_except.pt')
        #     print(abcd)
        ...
        #     print("\n\n@#@#@#$$$$$#@###@###@##@$$#@#\n\n")
        #     # print("@#@#@#@#@#@#@#@#@#@#@# i is ", type(i))
        #     print("@#@#@#@#@#@#@#@#@#@#@# i[0] keys is ", i[0].keys())
        #     print("@#@#@#@#@#@#@#@#@#@#printing for first element of tuple #@#@#@#@#@\n\n")
        #     print("@#@#@#@#@#@#@#@#@#@#@# rgb array shape is ", i[0]["rgb"].shape)  # array (128, 128, 3)
        #     print("@#@#@#@#@#@#@#@#@#@#@# depth array shape is ", i[0]["depth"].shape)     # array (128, 128, 1)
        #     print("@#@#@#@#@#@#@#@#@#@#@# mixed_bin_audio_mag array shape is ", i[0]["mixed_bin_audio_mag"].shape)  # array (512, 32, 2)
        #     print("@#@#@#@#@#@#@#@#@#@#@# gt_bin_comps array shape is ", i[0]["gt_bin_comps"].shape)   # array (512, 32, 8)
        #     print("@#@#@#@#@#@#@#@#@#@#@# gt_mono_comps array shape is ", i[0]["gt_mono_comps"].shape)   # array (512, 32, 4)
        #     print("@#@#@#@#@#@#@#@#@#@#@# target_class len is ", len(i[0]["target_class"]))   # an integer with value 0
        #     print("@#@#@#@#@#@#@#@#@#@#@# pose array shape is ", i[0]["pose"].shape)   # array (4, )

        #     print("#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#")
        #     print("@#@#@#@#@#@#@#@#@#@#printing for second element of tuple #@#@#@#@#@\n\n")
        #     print("type i[1] ", type(i[1]))   # int
        #     print("\n\n#@#@#@#@#@#@#@# i[1] is ", i[1]) # 0

        #     itr_ii += 1
        #     print("itr_ii is ", itr_ii)

        # except:
            # it is a list of tuples containing two elements.first entry of the tuple is a dict and second element is an int. initialize with random arrays. Added by Sanjoy  
            
            # b = dict()
            # b['rgb'] = np.random.rand(128, 128, 3)
            # b['depth'] = np.random.rand(128, 128, 1)
            # b['mixed_bin_audio_mag'] = np.random.rand(512, 32, 2)
            # b['gt_bin_comps'] = np.random.rand(512, 32, 8)
            # b['gt_mono_comps'] = np.random.rand(512, 32, 4)
            # b['target_class'] =  97
            # b['pose'] = [-0.,  1.,  0., 19.]
            # outputs = [(b,0), (b,0)]
        ...
        if True:
            observations, rewards, dones, infos = [list(x) for x in zip(*outputs)]   # expecting 4 outputs, but outputs contain 2 elements
        ...
        for reward_idx in range(len(rewards)):
            ...
            if ppo_cfg.use_negDist_reward:
                # print("h1")
                geo_rewards[reward_idx] = - batch["ground_truth_geodesic_distance"][reward_idx, 0].item()
            else:
            ...
            if ppo_cfg.use_nov_reward:
                # print("h1")
                rewards[reward_idx] = infos[reward_idx]["novelty_reward"]
            else:
    ...
    def train(self) -> None:
        ...
        if ppo_cfg.use_ddppo:
            ...
            # print("@#@#!#!@#!@#!#self.world_rank is ", self.world_rank)
            # print("@#@#!#!@#!@#!#self.local_rank is ", self.local_rank)
            # print(abcd)
        ...
        # print('*****&&&**********&&&&&&&&**********self.config.NUM_UPDATES: ', self.config.NUM_UPDATES)
        # print(abcd)
        ...
        with writer_obj as writer:
            for update in range(starting_update, int(self.config.NUM_UPDATES)):
                ...
                # print("@@@@ppo_cfg.num_mini_batch train", ppo_cfg.num_mini_batch)
    ...
    def _eval_checkpoint(
        ...
        Returns:
            None
        """
        random.seed(self.val_config.SEED)
        np.random.seed(self.val_config.SEED)
        torch.manual_seed(self.val_config.SEED)

        # map location CPU is almost always better than mapping to a CUDA device.
        if separate_eval:
            ckpt_dict = self.load_checkpoint(checkpoint_path, map_location="cpu")
        else:
            ckpt_dict = {"state_dict":self.agent.state_dict(), "config":self.config}

        # setting up config
        if self.val_config.EVAL.USE_CKPT_CONFIG:
        ...
        if len(config.EPS_SCENES) != 0:
            ...
            #print("@@@@@@@@@@@@@@@@@@@inisde ppo_trainer function at 1")
            ...
            for scene in config.EPS_SCENES:
                for episode in dataset_episodes:
                    if episode["scene_id"].split("/")[0] == scene:
                        ...
                        #print("@@@@@@@@@@@@@@@@@@@@@@@@@@inside scene - episode loop!")
        ...
        print("@@@@ppo_cfg.num_mini_batch eval", ppo_cfg.num_mini_batch)
        
        sepExtMem_cfg = ppo_cfg.TRANSFORMER_MEMORY
        num_past_steps_refinement = sepExtMem_cfg.num_past_steps_refinement

        poseEnc_cfg = sepExtMem_cfg.POSE_ENCODING
        #print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@after pose encoding")
        ...
        self.agent.load_state_dict(ckpt_dict["state_dict"])
        #print("@@@@@@@@@@@@@@@@@@@@@@@@@after loading ckpt dict ")
        self.actor_critic = self.agent.actor_critic
        #print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@after agent actor critic load ")
        self.actor_critic.eval()
        #print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@after actor critic eval ")
        self.agent.eval()
        #print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@after agent eval ")
        self.agent.actor_critic.eval()
        #print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@after actor critic eval ")
        ...
        #print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@after test recurrent hiddne states pol ")
        ...
        # print("@@@@@@@@@@@@@@@@@@ list(range(self.envs.num_envs)): ", list(range(self.envs.num_envs)))
        # print(abcd)
        #print("@@@@@@@@@@@@@@@@@after active envs")
        ...
        #print("@@@@@@@@@@@@@@@@@after compute eval metrics loop")
        ...
        while (
            ...
            if True:
                ...
                if step_count_all_processes[0] >= config.EVAL_EPISODE_COUNT:
                    step_count_all_processes[0] = 0
                ...
                if prev_ep_id != current_episode_id:
                    prev_ep_id = current_episode_id
                    ep_cnt += 1
                ...
                for env_idx in active_envs:
                    ...
                    if not_done_masks[env_idx].item() == 0:
                        ...
                        if (next_scene, next_episode_id) not in stats_episodes:
                            ...
                            num_episode_numbers_taken += 1
                            step_count_all_processes[env_idx] = 0
                            print("&&&&&&&&&&&&&&&&&&& reset len(stats_episodes), step_count_all_processes[env_idx])", len(stats_episodes), step_count_all_processes[env_idx])
                        else: # hard-reset
                            episode_count_all_processes[env_idx] = num_episode_numbers_taken + 1
                            num_episode_numbers_taken += 1
                            step_count_all_processes[env_idx] = 0
                            print("&&&&&&&&&&&&&&&&&&& HARD reset len(stats_episodes), step_count_all_processes[env_idx])", len(stats_episodes), step_count_all_processes[env_idx])
