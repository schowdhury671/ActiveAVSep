...
class AudioMem(nn.Module):
    ...
    def get_selfAttn_outFeats(self,
        ...
        try:
            ...
            B_ = sepExtMem_mono_feats.size(1)
            B_ = B_ // pred_mono_feats.size(0)
            pred_mono_feats = pred_mono_feats.repeat(B_,1)
            # print("@@@@@@@@@@@@@@ skip_feats", skip_feats.shape)
            skip_feats = skip_feats.repeat(B_,1,1,1)
